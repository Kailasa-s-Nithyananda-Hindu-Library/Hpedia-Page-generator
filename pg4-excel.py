...
def getTitle(row):
    if row[chapterNameCol] == '' and row[sectionNameCol] == '':
        ...
        ptitle= bookName + ' ' + str(row[chapterNameCol]) + ':' + str(row[sectionNameCol])  # title in as given in the sheet
    return ptitle
...
def main(argv):
    ...
    csvFile = ''
    try:
        opts, args = getopt.getopt(argv, "g:b:c:x:", ["GoogleSheetId=", "CsvFile", "BookName=", "Excel="])
    except getopt.GetoptError:
        print ('pg.py [ [-g] <GoogleSheetId> | -x <ExcelFile> | -c <CsvFile> ] [-b <BookName>]')
        sys.exit(2)
    for opt, arg in opts:
    ...
    if GOOGLE_SHEETS_FILE_ID:
        ...
        # import requests
        # workbook=load_workbook ( filename  csv_FILE)
        # wks_list = workbook.worksheets()
        # values=wks_list[0].get_all_values()
        # wks=wks_list[0].get_all_values()
        df = pd.read_csv(csvFile, header=None, engine='python', encoding="utf-16", skiprows=None)
        df.insert(0,'RowNumber','')
        df.insert(0, 'Section','')
        df.insert(0,'Chapter', csvFile)
        wks=df.values
    else:
